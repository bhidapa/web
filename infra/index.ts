...
// Monolithic server on EC2 running all websites
...
const websitesServerBackupVault = new aws.backup.Vault(
  ...
  {
    name: name('websites-server-backup-vault'),
    tags: { proj },
  },
);
const websitesServerBackupPlan = new aws.backup.Plan(
  'websites-server-backup-plan',
  {
    name: name('websites-server-backup-plan'),
    rules: [
      {
        ruleName: 'daily-backups',
        targetVaultName: websitesServerBackupVault.name,
...
const websitesServerBackupRole = new aws.iam.Role(
  'websites-server-backup-role',
  {
    name: name('websites-server-backup-role'),
    assumeRolePolicy: {
      Version: '2012-10-17',
      Statement: [
        {
...
new aws.iam.RolePolicyAttachment('websites-server-backup-role-policy', {
  role: websitesServerBackupRole.name,
  policyArn:
    'arn:aws:iam::aws:policy/service-role/AWSBackupServiceRolePolicyForBackup',
});
new aws.backup.Selection('websites-server-backup-selection', {
  name: name('websites-server-backup-selection'),
  planId: websitesServerBackupPlan.id,
  iamRoleArn: websitesServerBackupRole.arn,
...
const websitesServerDeployDocument = new aws.ssm.Document(
  'websites-server-deploy-document',
  {
    ...
    content: pulumi
      .all([
        fpmImage.repositoryUrl, // both nginxImage and fpmImage images have the same repositoryUrl
